...
class TrainingIterations:
    ...
    def run_step(self):
        ...
        if self.iteration%self.log_period_iterations == 0:
            self.time_prev  = self.time_now
            self.time_now   = time.time()

            #compute fps, and remaining time in hours
            dt                      = (self.time_now - self.time_prev)/self.log_period_iterations
            self.time_remaining     = (1.0 - self.filter_k)*self.time_remaining + self.filter_k*((self.iterations_count - self.iteration)*dt)/3600.0
        
        #episode done, update score per episode
        score_per_episode_+= reward
        if done:                
            score_per_episode = (1.0 - self.filter_k)*score_per_episode + self.filter_k*score_per_episode_
            score_per_episode_= 0.0
        ...
        if self.iteration > 0 and self.iteration%self.log_period_iterations == 0:
            print(log_str)

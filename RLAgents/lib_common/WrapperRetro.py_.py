...
class ActionsEnv(gym.Wrapper):
    def __init__(self, env=None):
        super(ActionsEnv, self).__init__(env)
        self.actions_count = env.action_space.shape[0]
    ...
    def step(self, action):
        action_one_hot = numpy.zeros(self.actions_count, dtype=int)
        
        if action < self.actions_count:
            action_one_hot[action] = 1
       
        return self.env.step(action_one_hot)
...
def WrapperRetro(env, height = 96, width = 96, frame_stacking=4, frame_skipping=4):
    env = ActionsEnv(env)
    env = NopOpsEnv(env)

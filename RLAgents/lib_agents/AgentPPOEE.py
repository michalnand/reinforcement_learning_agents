...
class AgentPPOEE():   
    def __init__(self, envs, ModelPPO, ModelRND, config):
        ...
        tmp     = numpy.concatenate([self.states, goal, goal], axis=1)
    ...
    def main(self):
        ...
        self.goals_buffer.add(rewards, self.rewards_episode_sum)
    ...
    def _compute_loss(self, states, logits, actions,  returns_ext, returns_int_a, returns_int_b, advantages_ext, advantages_int_a, advantages_int_b):
        ...
        loss_critic     = loss_ext_value + 0*loss_int_a_value + loss_int_b_value
 
        ''' 
        compute actor loss, surrogate loss
        '''
        advantages      = self.ext_adv_coeff*advantages_ext + 0*self.int_a_adv_coeff*advantages_int_a + self.int_b_adv_coeff*advantages_int_b

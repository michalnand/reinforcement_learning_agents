...
class AgentPPOEE():   
    def __init__(self, envs, ModelPPO, ModelRND, config):
        ...
        self.goals_buffer  = GoalsBuffer(config.goals_buffer_size, config.goals_add_threshold, config.goals_downsampling, self.goals_ext_reward_ratio, self.state_shape, self.parallel_envs, self.model_ppo.device)

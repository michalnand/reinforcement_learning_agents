...
class AgentPPOCuriosityActor():   
    ...
    def main(self):
        ...
        curiosity_np    = self._curiosity(states_t, actions)
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                features_predicted_t, features_target_t  = self.model_rnd(state_norm_t, action_one_hot_t)
    ...
    def _compute_loss(self, states, logits, actions,  returns_ext, returns_int, advantages_ext, advantages_int):
        ...
        '''
        compute curiosity motivation loss
        '''
        state_norm_t  = self._norm_state(states)
        features_predicted_t, features_target_t  = self.model_rnd(state_norm_t, probs_new)

        loss_curiosity    = -((features_target_t - features_predicted_t)**2).sum(dim=1)
        loss_curiosity    = loss_curiosity.mean()
       
        loss = 0.5*loss_critic + loss_policy + loss_entropy + self.curiosity_beta*loss_curiosity
    ...
    def _curiosity(self, state_t, action):
        state_norm_t            = self._norm_state(state_t)
        action_one_hot_t        = self._action_one_hot(numpy.array(action))

...
class AgentPPOCNDSAExtendedNew():   
    def __init__(self, envs, ModelPPO, ModelCND, config):
        ...
        if config.aux_loss == "action_loss":
            self._aux_loss = self._action_loss
        elif config.aux_loss == "constructor_loss":
             self._aux_loss = self._constructor_loss
        else:
            self._aux_loss = None

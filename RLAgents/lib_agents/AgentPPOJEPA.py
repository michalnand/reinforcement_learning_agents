...
class AgentPPOJEPA():   
    def __init__(self, envs, ModelPPO, ModelIM, config):
        ...
        if config.target_self_supervised_loss == "vicreg_jepa":
    ...
    def step(self, states, training_enabled, legal_actions_mask):        
        ...
        #state to tensor
        states_prev_t = self.states_t.clone()
        self.states_t = torch.tensor(states_norm, dtype=torch.float).to(self.device)

        #compute model output
        logits_t, values_ext_t, values_int_t = self.model_ppo.forward(self.states_t)
        ...
        if training_enabled:
            states_t        = self.states_t.detach().to("cpu")
    ...
    def train(self): 
        ...
        for batch_idx in range(batch_count):
            ...
            loss_im, im_ssl     = self._target_self_supervised_loss(self.model_im.forward_self_supervised, self._augmentations, states, states_next, 1.0)                

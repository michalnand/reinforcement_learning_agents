...
class AgentPPOSNDEEE():   
    def __init__(self, envs, ModelPPO, ModelSNDTarget, ModelSND, ModelCont, config):
        self.envs = envs  
        ...
        if config.cont_regularisation_loss == "mse":
            self._cont_regularisation_loss = self._contrastive_loss_mse
        elif config.cont_regularisation_loss == "info_nce":
            self._cont_regularisation_loss = self._compute_contrastive_loss_info_nce
        else:
            self._cont_regularisation_loss = None

        print("snd_regularisation_loss  = ", self._snd_regularisation_loss)
        print("ppo_regularisation_loss  = ", self._ppo_regularisation_loss)
        print("cont_regularisation_loss = ", self._cont_regularisation_loss)
        ...
        self.model_cont          = ModelCont.Model(self.state_shape)
    ...
    def main(self): 
        ...
        rewards_int_b       = numpy.clip(int_b_reward_coeffs*rewards_int_b, 0.0, 1.0)
    ...
    def get_log(self): 
        ...
        result+= str(round(self.log_loss_cont, 7)) + " "
    ...
    def train(self): 
        self.policy_buffer.compute_returns(self.gammas_ext, self.gammas_int_a, self.gammas_int_b)
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                states_a, states_b, labels = self.policy_buffer.sample_states(small_batch_size, self.model_ppo.device)
                ...
                if self._cont_regularisation_loss is not None:                    
                    loss = self._cont_regularisation_loss(self.model_cont, states_a, states_b, labels, normalise=False, augmentation=True)                
    
                    self.optimizer_cont.zero_grad() 
                    loss.backward()
                    self.optimizer_cont.step()

                    k = 0.02
                    self.log_loss_cont  = (1.0 - k)*self.log_loss_cont + k*loss.detach().to("cpu").numpy()
    ...
    def _compute_contrastive_loss_info_nce(self, model, states_a_t, states_b_t, target_t, normalise, augmentation):
        ...
        print(">>> ", logits.shape, target_t.shape)

...
class AgentPPOCuriosityFeatures():  
    def __init__(self, envs, ModelPPO, ModelForward, ModelForwardTarget, config):
        ...
        features            = self.model_ppo.forward_features(state_t) 

        self.model_forward          = ModelForward.Model(features.shape[1:])
        self.optimizer_forward      = torch.optim.Adam(self.model_forward.parameters(), lr=config.learning_rate_forward)

        self.model_forward_target   = ModelForwardTarget.Model(features.shape[1:])
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                features_target_t       = self.model_forward_target(features_t).detach()
                features_predicted_t    = self.model_forward(features_t)
    ...
    def _curiosity(self, state_t):
        features_t              = self.model_ppo.forward_features(state_t).detach()

        features_target_t       = self.model_forward_target(features_t)
        features_predicted_t    = self.model_forward(features_t)

...
class AgentPPOEntropy():   
    def __init__(self, envs, ModelPPO, config):
        ...
        features_count      = (self.state_shape[1]//self.downsample)*(self.state_shape[2]//self.downsample)
        ...
        self.features_buffer = FeaturesBuffer(config.features_buffer_size, self.envs_count, (features_count, ), self.model_ppo.device)
    ...
    def _curiosity(self, state_t):
        ...
        features_t      = torch.nn.functional.avg_pool2d(state_norm_t, kernel_size=self.downsample, stride=self.downsample)
        features_t      = features_t[:,0]
        features_t      = features_t.reshape((features_t.shape[0], features_t.shape[1]*features_t.shape[2]))

...
class AgentPPOSNDEEB():   
    def __init__(self, envs, ModelPPO, ModelSNDTarget, ModelSND, config):
        ...
        self.states_buffer  = StatesBuffer(config.buffer_size, state_shape_, config.buffer_add_threshold, config.buffer_downsample)
    ...
    def main(self): 
        ...
        #shorter path motivation and visitings count
        states_ = states[:,0,:,:]
        less_steps_reward, visitings_reward = self.states_buffer.update(states_, self.episode_steps_sum)

        int_b_reward_coeffs = numpy.take(self.int_b_reward_coeffs, self.modes)
        rewards_int_b       = numpy.clip(int_b_reward_coeffs*less_steps_reward, 0.0, 1.0)

        int_c_reward_coeffs = numpy.take(self.int_c_reward_coeffs, self.modes)
        rewards_int_c       = numpy.clip(int_c_reward_coeffs*visitings_reward, 0.0, 1.0)
    ...
    def save(self, save_path):
        ...
        self.states_buffer.save(save_path + "trained/")
    ...
    def get_log(self): 
        ...
        result+= str(round(self.states_buffer.get_usage(), 7)) + " "

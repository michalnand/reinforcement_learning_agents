...
class AgentPPOSSSND():   
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                states_now, states_next, states_similar, states_random, actions, relations = self.policy_buffer.sample_states_action_pairs(small_batch, self.device, self.similar_states_distance[i])

                #train snd target model, self supervised    
                loss_target_self_supervised = self._target_self_supervised_loss(self.model_snd, self._augmentations, states_now, states_next, states_similar, states_random, actions, relations)                

                if self._target_self_awareness_loss is not None:
                    loss_target_self_awareness, accuracy  = self._target_self_awareness_loss(self.model_snd, self._augmentations, states_now, states_next, states_similar, states_random, actions, relations)                
                else:
    ...
    def _loss_distillation(self, states): 
        features_target_t, features_predicted_t = self.model_snd(states)
    ...
    def _internal_motivation(self, states_prev, states):        
        ...
        prediction = self.model_snd.forward_aux(states_prev, states)

...
class AgentPPOSNDC():   
    def __init__(self, envs, Model, config):
        ...
        self.max_similarity_distance = config.max_similarity_distance
        ...
        if config.im_dist_loss == "metric_categorical":
        ...
        print("max_similarity_distance= ", self.max_similarity_distance)
        ...
        self.model      = Model.Model(self.state_shape, self.actions_count, self.max_similarity_distance+1)
    ...
    def train(self): 
        ...
        for batch_idx in range(batch_count):    
            ...
            if self._im_dist_loss is not None:
                sa, sb, distance = self.trajectory_buffer.sample_states_distance_pairs(self.ss_batch_size, self.max_similarity_distance, self.device)
                loss_distance, im_distance = self._im_dist_loss(self.model.forward_im_distance, sa, sb, distance)

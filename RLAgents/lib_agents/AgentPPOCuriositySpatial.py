...
class AgentPPOCuriositySpatial():
    def __init__(self, envs, ModelPPO, ModelForward, ModelForwardTarget, Config):
        ...
        state  = torch.zeros((1, ) + self.state_shape).to(self.model_forward.device)
        action = torch.zeros((1, self.actions_count)).to(self.model_forward.device)
        spatial, g = self.model_forward(state, action)
        ...
        self.internal_motivation_models = numpy.zeros(len(spatial) + 1)
    ...
    def main(self):
        ...
        curiosity_np    = self._curiosity(states_t, action_one_hot_t)

        curiosity_sum_np = numpy.sum(curiosity_np, axis=0)

        states, rewards, dones, _ = self.envs.step(actions)

        for e in range(self.actors):            
            if self.enabled_training:
                self.policy_buffer.add(e, states_np[e], logits_np[e], values_np[e], actions[e], rewards[e] + curiosity_sum_np[e], dones[e])
        ...
        self.internal_motivation            = (1.0 - k)*self.internal_motivation + k*curiosity_sum_np.mean()
        self.internal_motivation_models     = (1.0 - k)*self.internal_motivation_models + k*numpy.mean(curiosity_np, axis=1)
    ...
    def get_log(self):
        ...
        for i in range(len(self.internal_motivation_models)):
            result+= str(round(self.internal_motivation_models[i], 7)) + " "
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                spatial_predicted_t, global_predicted_t = self.model_forward(state_norm_t, action_one_hot_t)
                spatial_target_t, global_target_t       = self.model_forward_target(state_norm_t, action_one_hot_t)

                loss_forward = 0.0

                for i in range(len(spatial_predicted_t)):
                    loss_forward+= ((spatial_target_t[i].detach() - spatial_predicted_t[i])**2).mean()

                loss_forward+= ((global_target_t.detach() - global_predicted_t)**2).mean()
    ...
    def _curiosity(self, state_t, action_one_hot_t):
        ...
        models_count        = len(spatial_target_t) + 1
        curiosity_result    = torch.zeros((models_count, batch_size)).to(self.model_ppo.device)

        #spatial curiosity
        for i in range(len(spatial_predicted_t)):
            curiosity_t    = (spatial_target_t[i] - spatial_predicted_t[i])**2
            curiosity_t    = curiosity_t.mean(dim=1)

            #compute spatial curiosity, find maximum spatial value
            curiosity_t    = curiosity_t.view(curiosity_t.shape[0], -1).max(dim=1)[0]
            #squeeze values
            curiosity_result[i] = self.betas[i]*torch.tanh(curiosity_t)


        #global curiosity
        curiosity_t    = ((global_target_t - global_predicted_t)**2).mean(dim=1)
        #squeeze values
        curiosity_result[-1]    = self.betas[-1]*torch.tanh(curiosity_t)

        curiosity_np = curiosity_result.detach().to("cpu").numpy()

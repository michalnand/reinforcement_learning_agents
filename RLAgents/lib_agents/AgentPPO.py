...
class AgentPPO():
    def __init__(self, envs, Model, config):
        ...
        if self.rnn_policy:
            ...
            self.hidden_state+= 0.01*torch.randn((self.envs_count, ) + self.model.rnn_shape, device=self.device) 
    ...
    def step(self, states, training_enabled, legal_actions_mask):        
        ...
        if self.rnn_policy:
            ...
            for e in dones_idx:
                self.hidden_state[e] = 0.0 + 0.01*torch.randn(self.model.rnn_shape, device=self.device)

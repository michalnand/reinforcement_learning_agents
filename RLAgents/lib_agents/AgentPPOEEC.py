...
class AgentPPOEEC():   
    def __init__(self, envs, ModelPPO, ModelRND, config):
        ...
        self.policy_heads       = config.policy_heads
        ...
        self.model_ppo      = ModelPPO.Model(self.state_shape, self.actions_count, self.policy_heads)
        ...
        self.policy_buffer  = PolicyBufferIMTripple(self.steps, self.state_shape, self.goal_shape, self.actions_count, self.policy_heads, self.envs_count, self.model_ppo.device, True)
        ...
        self.log_heads_usage    = numpy.ones(self.policy_heads)/self.policy_heads
    ...
    def main(self): 
        ...
        policy, values_ext_a_t, values_int_a_t, values_ext_b_t, values_int_b_t = self.model_ppo.forward(states_t, goals_t, self.agent_mode)
        
        logits_t, head_logits_t, head_values_t, head_ids_np = policy
        ...
        head_logits_np      = head_logits_t.detach().to("cpu").numpy()
        head_values_np      = head_values_t.squeeze(1).detach().to("cpu").numpy()
        ...
        if self.enabled_training:
            ...
            self.policy_buffer.add_c(head_logits_np, head_values_np, head_ids_np, arbiter_reward, dones)
        ...
        heads_usage          = numpy.histogram(head_ids_np, bins=self.policy_heads, range=(0, self.policy_heads), density=True)[0]
    ...
    def get_log(self): 
        ...
        for h in range(self.policy_heads):
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                states, goals, modes, res_a, res_b, res_c = self.policy_buffer.sample_batch(self.batch_size, self.model_ppo.device)

                #train PPO model
                loss = self._compute_loss(states, goals, modes, res_a, res_b, res_c)
    ...
    def _compute_loss(self, states, goals, modes, res_a, res_b, res_c):
        ...
        policy_new, values_ext_a_new, values_int_a_new, values_ext_b_new, values_int_b_new  = self.model_ppo.forward(states, goals, modes)

        logits_new          = policy_new[0]
        arbiter_logits_new  = policy_new[1]
        arbiter_values      = policy_new[2]

        
        probs_new     = torch.nn.functional.softmax(logits_new, dim = 1)
        log_probs_new = torch.nn.functional.log_softmax(logits_new, dim = 1)
        
        arbiter_probs_new     = torch.nn.functional.softmax(arbiter_logits_new, dim = 1)
        arbiter_log_probs_new = torch.nn.functional.log_softmax(arbiter_logits_new, dim = 1)
        ...
        #arbiter loss        
        loss_arbiter = ((arbiter_returns.detach() - arbiter_values.squeeze(1))**2).mean()
        loss_arbiter+= self._actor_loss(arbiter_advantages, arbiter_probs_new, arbiter_log_probs_new, arbiter_log_probs_old, arbiter_actions)

        loss = 0.5*loss_critic + loss_actor + loss_arbiter
    ...
    def _actor_loss(self, advantages, probs_new, log_probs_new, log_probs_old, actions):        
        ''' 

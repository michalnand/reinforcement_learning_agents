...
from .PolicyBufferIMMode    import *  
...
class AgentPPOEED():   
    def __init__(self, envs, ModelPPO, ModelRND, config):
        ...
        self.policy_buffer  = PolicyBufferIMMode(self.steps, self.state_shape, self.actions_count, self.envs_count, self.model_ppo.device, True)
        ...
        self.episode_rewards_max = 0.0
        self.agent_modes         = numpy.zeros(self.envs_count)
    ...
    def main(self): 
        ...
        modes_t     = torch.tensor(self.agent_modes, dtype=torch.float).to(self.model_ppo.device)

        #compute model output
        logits_t, values_ext_t, values_int_t  = self.model_ppo.forward(states_t, modes_t)
        ...
        if self.enabled_training:
            self.policy_buffer.add(states_np, self.agent_modes, logits_np, values_ext_np, values_int_np, actions, rewards_ext, rewards_int, dones)

            if self.policy_buffer.is_full():
                self.train()
        
        for e in range(self.envs_count): 
            #determine agent mode, depends if already reached maximum known reward
            if self.episode_rewards_sum[e] >= self.episode_rewards_max:
                k = 0.1
                self.episode_rewards_max = (1.0 - k)*self.episode_rewards_max + k*self.episode_rewards_sum[e]
                self.agent_modes[e] = 1

            if dones[e]:
                self.episode_rewards_sum[e] = 0.0
                self.agent_modes[e]         = 0
                self.states[e]              = self.envs.reset(e).copy()

        #collect stats
        k = 0.02
        self.log_internal_motivation = (1.0 - k)*self.log_internal_motivation + k*rewards_int.mean()
        self.log_modes = (1.0 - k)*self.log_modes + k*self.agent_modes.mean()
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                states, modes, logits, actions, returns_ext, returns_int, advantages_ext, advantages_int = self.policy_buffer.sample_batch(self.batch_size, self.model_ppo.device)

                #train PPO model
                loss = self._compute_loss(states, modes, logits, actions, returns_ext, returns_int, advantages_ext, advantages_int)
    ...
    def _compute_loss(self, states, modes, logits, actions, returns_ext, returns_int, advantages_ext, advantages_int):
        log_probs_old = torch.nn.functional.log_softmax(logits, dim = 1).detach()

        logits_new, values_ext_new, values_int_new  = self.model_ppo.forward(states, modes)

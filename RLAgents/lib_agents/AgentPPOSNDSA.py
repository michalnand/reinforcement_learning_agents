...
class AgentPPOSNDSA():   
    def __init__(self, envs, ModelPPO, ModelSNDTarget, ModelSND, ModelSA, config):
        self.envs = envs  
    ...
    def get_log(self): 
        ...
        result+= str(round(self.loss_sa, 7)) + " "
        ...
        result+= str(round(self.log_acc, 7)) + " "
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                #train inverse attentive model
                action_predicted = self.model_sa(states, states_next)

                action_target    = torch.zeros_like(action_predicted)
                action_target[range(action_target.shape[0]), actions] = 1.0

                
                loss = ((action_target - action_predicted)**2).mean()
                
                self.optimizer_sa.zero_grad() 
                loss.backward()
                self.optimizer_sa.step()

                hits = torch.max(action_target, dim=1)[1] == torch.max(action_predicted, dim=1)[1]
                acc = torch.sum(hits)/hits.shape[0]

                k = 0.02
                self.loss_sa  = (1.0 - k)*self.loss_sa + k*loss.detach().to("cpu").numpy()
                self.log_acc  = (1.0 - k)*self.log_acc + k*acc.detach().to("cpu").numpy()
    ...
    def _entropy(self, state_t):
        features_t = self.model_sa.forward_features(state_t)
        self.features_buffer.add(features_t)

        entropy = self.features_buffer.compute_entropy()

        return entropy.detach().to("cpu").numpy()

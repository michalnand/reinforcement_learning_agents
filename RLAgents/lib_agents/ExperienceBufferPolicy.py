...
class ExperienceBufferPolicy():
    ...
    def _initialize(self):
        if self.initialized == False:
            ...
            self.probs_b        = numpy.zeros((self.size, self.actions_count), dtype=numpy.float32)
    ...
    def add(self, state, action, probs, reward, done, reward_int = 0.0): 
        self._initialize()
        ...
        self.probs_b[self.current_idx]      = probs.copy()
    ...
    def sample(self, batch_size, device = "cpu"):
        ...
        probs_t         = torch.from_numpy(numpy.take(self.probs_b,     indices, axis=0)).to(device)
        ...
        return state_t, state_next_t, action_t, probs_t, reward_t, done_t, reward_int_t

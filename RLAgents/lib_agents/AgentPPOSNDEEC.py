import numpy
import torch 
from .PolicyBufferIMModes   import *  
...
class AgentPPOSNDEEC():   
    def __init__(self, envs, ModelPPO, ModelSNDTarget, ModelSND, config):
        self.envs = envs  
      
        self.gammas_ext         = config.gammas_ext 
        self.gammas_int         = config.gammas_int
            
        self.ext_adv_coeff          = config.ext_adv_coeff
        self.int_adv_coeff          = config.int_adv_coeff

        self.int_a_reward_coeffs    = config.int_a_reward_coeffs
        self.int_b_reward_coeffs    = config.int_b_reward_coeffs
        self.int_c_reward_coeffs    = config.int_c_reward_coeffs
        ...
        self.policy_buffer  = PolicyBufferIMModes(self.steps, self.state_shape, self.actions_count, self.envs_count, self.model_ppo.device, True)

        state_shape_        = (1, self.state_shape[1], self.state_shape[2])
        self.goals_buffer   = GoalsBuffer(config.buffer_size, state_shape_, config.buffer_reach_threshold, config.mastering_threshold)
        ...
        self.episode_steps_sum = numpy.ones(self.envs_count)
        ...
        self.log_internal_motivation_c_mean   = 0.0
        self.log_internal_motivation_c_std    = 0.0

        self.log_mastered                     = 0.0
        self.log_modes                        = 0.0
    ...
    def main(self): 
        ...
        #accumulate episode steps
        self.episode_steps_sum+= 1
        ...
        int_a_reward_coeffs = numpy.take(self.int_a_reward_coeffs, self.modes)
        rewards_int_a       = numpy.clip(int_a_reward_coeffs*rewards_int_a, 0.0, 1.0)

        #goal reaching motivation
        states_ = states[:,0,:,:]
        reached_reward, less_steps_reward, goals_dones = self.goals_buffer.update(states_, self.episode_steps_sum, self.episode_score_sum)

        int_b_reward_coeffs = numpy.take(self.int_b_reward_coeffs, self.modes)
        rewards_int_b       = numpy.clip(int_b_reward_coeffs*reached_reward, 0.0, 1.0)

        int_c_reward_coeffs = numpy.take(self.int_c_reward_coeffs, self.modes)
        rewards_int_c       = numpy.clip(int_c_reward_coeffs*less_steps_reward, 0.0, 1.0)

        #total internal reward
        rewards_int = rewards_int_a + rewards_int_b + rewards_int_c
        ...
        if self.enabled_training:
            self.policy_buffer.add(states_np, logits_np, values_ext_np, values_int_np, actions, rewards_ext, rewards_int, dones, self.modes)
        ...
        for e in range(self.envs_count): 
            if dones[e] or (goals_dones[e] and self.enabled_training == False):
                self.states[e] = self.envs.reset(e).copy()
                self.modes[e]  = 0
                self.episode_score_sum[e] = 0
                self.episode_steps_sum[e] = 0
        ...
        self.log_internal_motivation_c_mean   = (1.0 - k)*self.log_internal_motivation_c_mean + k*rewards_int_c.mean()
        self.log_internal_motivation_c_std    = (1.0 - k)*self.log_internal_motivation_c_std  + k*rewards_int_c.std()
    ...
    def get_log(self): 
        ...
        result+= str(round(self.log_internal_motivation_c_mean, 7)) + " "
        result+= str(round(self.log_internal_motivation_c_std, 7))  + " "

        result+= str(round(self.log_mastered, 7)) + " "
        result+= str(round(self.log_modes, 7)) + " "
    ...
    def train(self): 
        self.policy_buffer.compute_returns(self.gammas_ext, self.gammas_int)

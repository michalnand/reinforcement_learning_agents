...
class AgentPPOSNDAdvB():   
    def __init__(self, envs, Model, config):
        ...
        self.im_hidden_state = torch.zeros((self.envs_count, 2, self.model.rnn_size) , dtype=torch.float32, device=self.device)
    ...
    def step(self, states, training_enabled, legal_actions_mask):        
        ...
        #internal motivation
        novelty_spatial, novelty_temporal, im_hidden_state_new = self._internal_motivation(states_t, self.im_hidden_state)
        rewards_int = self.reward_int_a_coeff*novelty_spatial + self.reward_int_b_coeff*novelty_temporal
        ...
        if training_enabled:
            ...
            hidden_state    = self.im_hidden_state.detach().to("cpu")
        ...
        self.im_hidden_state = im_hidden_state_new.detach().clone()        
        ...
        #collect stats
        self.values_logger.add("internal_motivation_a_mean", novelty_spatial.mean().detach().to("cpu").numpy())
        self.values_logger.add("internal_motivation_a_std" , novelty_spatial.std().detach().to("cpu").numpy())
        self.values_logger.add("internal_motivation_b_mean", novelty_temporal.mean().detach().to("cpu").numpy())
        self.values_logger.add("internal_motivation_b_std" , novelty_temporal.std().detach().to("cpu").numpy())
    ...
    def _internal_motivation(self, states, hidden_state):        
        z_spatial_target    = self.model.forward_im_spatial_target(states)
        z_spatial_predictor = self.model.forward_im_spatial_predictor(states)

        novelty_spatial     = ((z_spatial_target.detach() - z_spatial_predictor)**2).mean(dim=1)

        z_target_temporal, h_new_target       = self.model.forward_im_temporal_target(z_target, hidden_state[:, 0])
        z_predictor_temporal, h_new_predictor = self.model.forward_im_temporal_predictor(z_predictor, hidden_state[:, 1])

        novelty_temporal = ((z_target_temporal.detach() - z_predictor_temporal)**2).mean(dim=1)

        hidden_state_new = torch.concatenate([h_new_target.unsqueeze(1), h_new_predictor.unsqueeze(1)], dim=0)

        return novelty_spatial, novelty_temporal,  hidden_state_new

...
class PolicyBufferIMDualModes:
    ...
    def clear(self):
        ...
        self.modes          = numpy.zeros((self.buffer_size, self.envs_count, ), dtype=int)

        self.ptr = 0  
 

    def compute_returns(self, gammas_ext, gammas_int_a, gammas_int_b, lam = 0.95):

        gamma_ext   = numpy.take(gammas_ext, self.modes)
        gamma_int_a = numpy.take(gammas_int_a, self.modes)
        gamma_int_b = numpy.take(gammas_int_b, self.modes)
